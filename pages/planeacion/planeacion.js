document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateStepButtonState(stepElement) {
    ...
    const hasSelection = !!stepElement.querySelector(
  ...
  function recalculateTotal() {
    ...
    if (Object.keys(summaryByStep).length === 0) {
      summaryList.innerHTML =
        '<li class="text-muted">Aún no has seleccionado opciones.</li>';
    } else {
  ...
  const disposicionRadios = document.querySelectorAll(
    "input[name='disposicion']",
  );
  const optionGroupsByDisposition = document.querySelectorAll(
    ".option-group[data-for]",
  );
  ...
  disposicionRadios.forEach((radio) => {
    radio.addEventListener("change", function (e) {
      ...
      // Al cambiar disposición, limpia todos los pasos siguientes
      const currentIndex = steps.indexOf(step);
      if (currentIndex >= 0) {
        for (let i = currentIndex + 1; i < steps.length; i++) {
          const laterInputs = steps[i].querySelectorAll(
  ...
  groupsWithExtras.forEach((group) => {
    ...
    btnLess.type = "button";
    btnLess.className =
      "btn btn-sm btn-outline-secondary ver-menos-opciones d-none";
    ...
    btnMore.addEventListener("click", function () {
      for (
        let i = 0;
        i < pageSize && visibleExtras < extraOptions.length;
        i++
      ) {
